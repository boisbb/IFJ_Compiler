  ...
  while (1) {
    ...
    switch (PRECED_TABLE[stack.top->prec_tab_id][token_index]) {
      ...
      case '<':
        ...
        break;
      case '>':
        // Poping from stack and generating instruction based on operator/operand
...
int check_operators_and_operands_syntax(Type operator){
  ...
  else if ((l_operand.type == TypeString && r_operand.type == TypeInt) || (l_operand.type == TypeInt && r_operand.type == TypeString)){
    if (operator != TypeOperatorMul) {
      DEBUG_PRINT("SYNTAX ERROR: Incorrect operator.\n");
      return ERROR_SYNTAX;
    }
    result = TypeString;
